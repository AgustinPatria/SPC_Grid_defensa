...
`build_dl_context` is the bridge: it mixes **historical** `mu_hat/sigma_hat` (per `(i, j, regime)`) with **DL-forecasted** `p_dl(t)` to build `mu_mix/sigma_mix(t)` over the forward horizon (t=1..T_HORIZON), and generates the 5 representative scenarios. The returned dict is drop-in compatible with `solve_portfolio` (same shape as `load_market_data`'s output) plus extra keys `scenarios`, `p_dl`, `p_hist`.

`build_dl_context` has two PDF-alignment knobs (defaults validados via auditoria — ver `HALLAZGOS_ALINEAMIENTO_PDF.md`):
- `p_method='walking'` (default) aplica el LSTM a ventanas REALES `r_hist[t-H..t-1]` para construir `p_dl(t)`. Captura variacion temporal del modelo. **Validado con synthetic_walking_test**: rollout determinístico (que parece pedir el PDF sec. 2.5) **colapsa a un punto fijo por exposure bias** — incluso con data sintetica de senal clara (p_hist std=0.18) produce `p_dl=0.60` constante. Walking, en cambio, sigue la senal (p_dl std=0.17 sobre la misma sintetica). Tambien existen `'rollout'` (autoregresivo deterministico, colapsa) y `'scenarios'` (promedio sobre N candidatos).
- `mu_hat_source='p_sign'` (default) estima mu_hat con un **oraculo historico** que usa la MISMA regla de regimen que el LSTM (PDF sec. 2.4 ec. 15: `bull si r >= BULL_THRESHOLD`). Resuelve la inconsistencia entre la definicion del estimador y la del LSTM (con `p_hist` daba mu_hat(bull) ≈ mu_hat(bear) con signo invertido). Tambien existe `'p_hist'` (lectura literal del PDF sec. 1.3) y `'p_dl'` (legacy).
- Para reproducir el codigo original: `build_dl_context(..., p_method='walking', mu_hat_source='p_dl')`. Para la lectura literal del PDF (sec 1.3 ec. 2 con p del CSV): `mu_hat_source='p_hist'`. Los experimentos `padding_ablation`, `leakage_check` y `muhat_leakage` estan pinned al legacy porque su pregunta solo aplica ahi.
- **Reporte comparativo HALLAZGOS sec 9-bis**: los tres setups (`p_hist` PDF literal, `p_sign` propuesta propia, `p_dl` legacy) corren con `python inspeccion/main_pdf_literal.py` (literal) y `python main.py` (propuesta) — outputs en `inspeccion/main_pdf_literal_out/` y `resultados/` respectivamente. Backtests historicos para los tres: −20.62% (p_hist), −32.27% (p_sign), −13.20% (p_dl).
- **Resultado validacion**: con data sintetica de senal clara (sinusoide period=30), RG con walking+psign hace **+103%** vs naive −26% — el framework FUNCIONA. Con data real, RG hace **−32.27%** porque la data real no tiene senal temporal extractable por un LSTM cuantilico con 163 semanas. El cuello del pipeline son los datos, no la formulacion.
